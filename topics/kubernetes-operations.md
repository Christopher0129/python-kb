# Kubernetes 运维专题

这个专题关注 Python 服务运行在 Kubernetes 上时的部署、配置、伸缩和排障。

## 常见主题

- Deployment / Service 基本结构
- ConfigMap / Secret 使用
- Job / CronJob
- 健康检查与滚动发布
- 日志与资源限制

## 关联阅读

- [Kubernetes Python Client](../third_party/kubernetes.md)
- [发布策略专题](./deployment-strategies.md)
- [健康检查与指标专题](./health-metrics.md)
- [Kubernetes 自动伸缩专题](./kubernetes-autoscaling.md)
- [Kubernetes 探针与优雅停机专题](./kubernetes-probes-graceful-shutdown.md)
- [Kubernetes 资源治理专题](./kubernetes-resource-management.md)
- [Kubernetes 滚动交付专题](./kubernetes-rolling-delivery.md)
- [Kubernetes PDB 与可用性专题](./kubernetes-pdb-availability.md)
- [Kubernetes 调度与亲和性专题](./kubernetes-scheduling-affinity.md)
- [Kubernetes Ingress 与流量入口专题](./kubernetes-ingress-traffic.md)
- [Kubernetes 多集群专题](./kubernetes-multi-cluster.md)
- [Kubernetes Stateful 工作负载专题](./kubernetes-stateful-workloads.md)
- [Kubernetes Job 与 CronJob 专题](./kubernetes-cronjobs-jobs.md)
- [Kubernetes NetworkPolicy 专题](./kubernetes-network-policies.md)
- [Kubernetes Secret 分发专题](./kubernetes-secret-distribution.md)
- [Kubernetes ConfigMap 与运行时配置专题](./kubernetes-configmaps-runtime-config.md)
- [Kubernetes 工作负载身份专题](./kubernetes-workload-identity.md)
- [Kubernetes ServiceAccount 与 RBAC 专题](./kubernetes-serviceaccount-rbac.md)
- [Kubernetes 准入控制专题](./kubernetes-admission-control.md)
- [Kubernetes 镜像拉取故障专题](./kubernetes-image-pull-failures.md)
- [Kubernetes Pod 排障专题](./kubernetes-pod-debugging.md)
- [Kubernetes 控制器排障专题](./kubernetes-controller-debugging.md)
- [Kubernetes 驱逐与抢占专题](./kubernetes-evictions.md)
- [Kubernetes 优先级与抢占专题](./kubernetes-priority-preemption.md)
- [Kubernetes 回滚专题](./kubernetes-rollbacks.md)
- [Kubernetes 节点池专题](./kubernetes-node-pools.md)
- [Kubernetes 多租户专题](./kubernetes-multi-tenancy.md)
- [Kubernetes 配置漂移专题](./kubernetes-gitops-drift.md)
- [Kubernetes 成本优化专题](./kubernetes-cost-optimization.md)
- [部署与运维专题](./deployment-operations.md)
- [Helm 与 Chart 专题](./helm-charts.md)
- [GitOps 交付专题](./gitops-delivery.md)

返回 [索引](../README.md)
